...
const updateUser = async (req, res) => {
  ...
  if (!req.body.currentPassword) {
    res.status(422).send({
      error: "you need to send your currentPassword to update your user",
    });
    return;
  }
  ...
  user.set({
    ...newValues,
    password: req.body.newPassword || req.body.currentPassword,
    passwordHash: null,
  });
...
const getUser = async (req, res) => {
  ...
  res.status(200).json({
    avatar: "",
    username: "medic1111",
    Posts: [{ title: "one", content: "testing" }],
    biography: "This is medic's bio",
  });
};
